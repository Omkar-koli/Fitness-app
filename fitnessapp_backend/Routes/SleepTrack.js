...
router.post('/addsleepentry', authTokenHandler, async (req, res) => {
    ...
    if (!date || !durationInHrs  ){
        return res.status(400).json(createResponse(false, 'Please provide data and sleep duration'));
    }
    const userId = req.userId;
    const user = await User.findById({ _id: userId });

    const existingEntry = user.sleep.find(entry => {
        return filterEntriesByDate(new Date(entry.date), new Date(date));

    });
    if (existingEntry) {
        existingEntry.duration = durationInHrs;
        await user.save();
    }
    user.sleep.push({
        date : new Date(date),
...
router.post('/getsleepbylimit', authTokenHandler, async (req, res) => {
    ...
    if(!limit) {
        ...
        return res.json(createResponse(true, `Sleep entries for the late ${limit} days`, user.sleep));
